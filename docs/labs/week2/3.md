...
Remember, to declare a variable or give it a value you use the  `=` symbol, like so:
...
```
### ⚽Class Exercise #3 - Turning hardcode to variables

Replace the hard coded values of `const map = L.map('the_map').setView([34.0709, -118.444], 5);` with variables.
...
??? done "Answer"
    ...
    // const map = L.map('the_map').setView(myMap.center, myMap.zoom);
    ```

!!! warning "No spaces in variable names!"
    You **cannot** use spaces in variable definitions like `let my map;`, so stick with `camelCase`.

!!! question "Reflection"
    Think about the benefits of having the variables sitting outside like that, is it easier to read for you? Harder?

### Working with our Dev Console
...
### ⚽Class Exercise #4 - Using the marker function
...
This technique will be helpful for our pop-ups next week.
### 🏁Checkpoint
